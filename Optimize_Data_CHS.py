...
def CURVE_VS_SAMP(sheet, my_x, samples ):
    ...
    #image to corrected excel
    img = Image(infile_name + ' cvs ' + sheet + ".png", size = (300,300))
    img.anchor(wb[sheet]['N5'])
    wb[sheet].add_image(img)
    #image to CHS_GRAPHS.xlsx
    img.anchor(CHS_GRAPHS[sheet]['T1'])
    CHS_GRAPHS[sheet].add_image(img)
    plt.close()
def CURVE (sheet, my_x, my_y, best_poly):
    ...
    #add to corrected xlsx
    img2.anchor(wb[sheet]['S5'])
    wb[sheet].add_image(img2)
    #add to chs_graphs
    img2.anchor(CHS_GRAPHS[sheet]['N1'])
    CHS_GRAPHS[sheet].add_image(img2)
    plt.close()
    #exp vs obt rygb
    #~~~~good~~~~#
...
#for pasting into CHS_GRAPHS.xlsx
CHS_GRAPHS = load_workbook(path2 + "/CHS_GRAPHS.xlsx")
CHS_GRAPH_ANCHOR = 1
##if Good_to_Go == True:
    ...
    #####read the appropriate cells into each list
        ...
        for i in range(row_of_curve, row_of_curve + 8):
            ...
            if i == row_of_curve + 7:
                ...
                data_from = ws_from.cell(row = i, column = col_of_dil).value
        ...
        for i in range(0,40):
            if sample_1[i] == 'OVRFLW' and sample_2[i] != 'OVRFLW':
            ...
            elif sample_2[i] == 'OVRFLW' and sample_1[i] != 'OVRFLW':
                sample_2[i] = sample_1[i]
                outfile.write(sheet+" Sample Overflow: Sample Number: " + str(i + 1) + " absorbance replaced with first measured absorbance " + str(sample_2[i]) + "\n")
     ...
     #####make sure curve values are in the proper order#####           
        ...
        while (k < len(curve_1) - 1):
            if(curve_1[k] < curve_1[k +1]):
                ...
                del (curve_yval_1[k])
                del (dil[k])
                outfile.write(sheet+" Curve #1 Descent Issue: Point " + str (k +1) + " was removed\n")
            else:
                k += 1
                
               
        h = 0
        while (h < len(curve_2) - 1):
            if(curve_2[h]) < curve_2[h + 1]:
               ...
               del (curve_yval_1[h])
               del (dil[h])
               outfile.write(sheet+" Curve #2 Descent Issue: Point " + str(h +1) + " was removed\n")
            else:
               h += 1

        if curve_1[len(curve_1)-1]> curve_1[len(curve_1)-2] or curve_2[len(curve_2)-1]> curve_2[len(curve_2)-2] :
            print("Think about manipulating bottom points")
...
##################CLEAR THIS ALL UP FOR THE VALUES TOO HIGH OR LOW ####################
        ...
        ###### Make Graphs ######       
            ...
            if max(expCor) != 0:
                ...
                if (best_m) > 1.5:
                    outfile.write(sheet + " : Obtained Values From Standards Lower Than Expected (m > 1.5): " + str(best_m) + "\n")
                elif (best_m)< 0.5:
                    outfile.write(sheet + " : Obtained Values From Standards Higher Than Expected (m < 0.5): " + str(best_m) + "\n")
    ...
    for lipid in lipids:
        ...
        if (best_m) > 1.5:
            outfile.write(lipid + " : Obtained Values From Standards Lower Than Expected (m > 1.5): " + str(best_m) + "\n")
        elif (best_m)< 0.5:
            outfile.write(lipid + ": Obtained Values From Standards Higher Than Expected (m < 0.5): " + str(best_m) + "\n")
        #paste things in
        #obt and cor
        j = 0
        for i in range(25,29):
